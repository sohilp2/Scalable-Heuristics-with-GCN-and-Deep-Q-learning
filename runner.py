        ...
        for g in range(games):
            ...
            for i in range(1, max_iter + 1): #mat iter:
                ...
                if self.test and done:
                    ...
                    list_optimal_ratio.append((optimal_sol))
...
#                     list_aprox_ratio.append(cumul_reward/(approx_sol))
        ...
        if self.test:
            ...
            fig = plt.figure()
            plt.plot(list_optimal_ratio)
            plt.ylabel("Optimal Ratio")
            plt.xlabel("Games")
            plt.show()
            fig.savefig('solution.png', dpi=fig.dpi)
            print("Obtained a final reward of {}".format(cumul_sol/(games+1)))
            print("Obtained a performance (max ratio) of {}".format(max_ratio))
